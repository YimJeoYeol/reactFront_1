...
const FileManager = (props) => {
    ...
    const fileDelete = async (boardId, fileId) => {
        try {
            const response = await axios.delete(`http://localhost:8686/board/${boardId}/file/delete?fileId=${fileId}`, {headers: headers});
        ...
        <div className='file-box'>
            ...
            <span>
              <strong>File Name:</strong> {file.originFileName} &nbsp;
                {/* 파일 다운로드 버튼 */}
                <a href={`http://localhost:8686/board/${boardId}/file/download?fileId=${file.fileId}`} download>
                Download
              </a>
            </span>
                        {/* 삭제 버튼을 가장 오른쪽에 배치하기 */}
