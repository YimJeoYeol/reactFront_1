...
function BbsDetail() {
  ...
  const getBbsDetail = async () => {
    try {
      const response = await axios.get(`http://localhost:8888/board/${boardId}`);
  ...
  const deleteBbs = async () => {
    try {
      const response = await axios.delete(`http://localhost:8888/board/${boardId}/delete`, {headers: headers});
